        ...
        <div className="flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
            ...
            {appState.stage === 'idle' && (
                <div className="w-full overflow-x-auto pb-4">
                    ...
                    >
                        <Uploader id="product-upload" onUpload={handleProductImageUpload} onImageChange={handleProductImageChange} caption={uploaderCaptionProduct} description={uploaderDescriptionProduct} currentImage={appState.productImage} placeholderType="clothing" cardType="content-input" />
                        <Uploader id="scene-upload" onUpload={handleSceneImageUpload} onImageChange={handleSceneImageChange} caption={uploaderCaptionScene} description={uploaderDescriptionScene} currentImage={appState.sceneImage} placeholderType="style" cardType="style-input" />
                    </motion.div>
                </div>
            )}
            ...
            {appState.stage === 'configuring' && appState.productImage && appState.sceneImage && (
                 <motion.div className="flex flex-col items-center gap-8 w-full max-w-screen-2xl py-6 overflow-y-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                    ...
                    <OptionsPanel className="max-w-3xl">
                        ...
                        />
                        <div>
                            <label htmlFor="notes-studio" className="block text-left base-font font-bold text-lg text-neutral-200 mb-2">{t('productStudio_promptLabel')}</label>
                            <textarea
                                id="notes-studio"
                                value={localNotes}
